...
document.getElementById("upload-form").addEventListener("submit", async (e) => {
    ...
    if (!file && !content) {
        alert("Please upload a document or write content.");
        return;
    }
...
async function search(query) {
  ...
  function displaySearchResults(results) {
    ...
    results.forEach(result => {
      ...
      listItem.innerHTML = `<strong>Text:</strong> ${text}<br>`;
      
      if (entities.length > 0) {
        listItem.innerHTML += `<strong>Entities:</strong><br>`;
        entities.forEach(entity => {
          listItem.innerHTML += `${entity[0]} (${entity[1]})<br>`;
        });
      } else {
        listItem.innerHTML += "<em>No entities found</em>";
      }
